...
class MotorManagerUI(QWidget):
    """Motor Instanceの登録・編集・履歴・Benchmark RPM・比較を管理するUI。"""
    ...
    def __init__(self):
        ...
        self.benchmark_repo = BenchmarkResultRepository(self.db)
    ...
    def setup_instance_tab(self):
        ...
        self.status_box = QComboBox(); self.status_box.addItems(["NEW", "ACTIVE", "MAINTENANCE", "RETIRED", "ARCHIVED"]); form.addRow("Status", self.status_box)
        self.health_box = QComboBox(); self.health_box.addItems(["UNKNOWN", "GOOD", "WARNING", "BAD"]); form.addRow("Health", self.health_box)
        self.purchase_edit = QLineEdit(); self.purchase_edit.setPlaceholderText("YYYY-MM-DD"); form.addRow("Purchase Date", self.purchase_edit)
        self.opened_edit = QLineEdit(); self.opened_edit.setPlaceholderText("YYYY-MM-DD"); form.addRow("Opened Date", self.opened_edit)
        layout.addWidget(box)

        buttons = QHBoxLayout()
        self.save_button = QPushButton("Register"); self.save_button.clicked.connect(self.save_instance); buttons.addWidget(self.save_button)
        new_button = QPushButton("New"); new_button.clicked.connect(self.clear_form); buttons.addWidget(new_button)
        delete_button = QPushButton("Retire / Delete"); delete_button.clicked.connect(self.delete_instance); buttons.addWidget(delete_button)
        buttons.addStretch(); layout.addLayout(buttons)

        self.instance_table = QTableWidget()
        self.instance_table.setColumnCount(11)
        self.instance_table.setHorizontalHeaderLabels(["ID", "Model", "Serial", "Nickname", "Status", "Health", "Latest Session", "Benchmark RPM", "Anomaly", "Created", "Updated"])
        ...
        layout.addWidget(QLabel("Double-click an instance to edit/view history. Select multiple rows for comparison."))
    ...
    def setup_detail_tab(self):
        ...
        self.detail_info = QTableWidget(); self.detail_info.setColumnCount(2); self.detail_info.setHorizontalHeaderLabels(["Field", "Value"]); self.detail_info.setEditTriggers(QTableWidget.NoEditTriggers); layout.addWidget(self.detail_info)

        layout.addWidget(QLabel("Measurement / Break-in History"))
        self.history_table = QTableWidget(); self.history_table.setColumnCount(10)
        self.history_table.setHorizontalHeaderLabels(["Session", "Device", "Device Model", "Start", "End", "Result", "Logs", "Measured RPM", "Benchmark RPM", "Last Current mA"])
        self.history_table.setEditTriggers(QTableWidget.NoEditTriggers); layout.addWidget(self.history_table)

    def setup_compare_tab(self):
        layout = QVBoxLayout(self.compare_tab)
        row = QHBoxLayout(); row.addWidget(QLabel("Selected Motor Instances"))
    ...
    def load_instances(self):
        ...
        for r, data in enumerate(rows):
            ...
            values = [data.get("instance_id"), data.get("motor_name", data.get("motor_model_id")), data.get("serial_number"), data.get("nickname"), data.get("status"), data.get("health_status"), data.get("latest_session_id"), benchmark.get("benchmark_rpm") if benchmark else None, data.get("anomaly_count", 0), data.get("created_at"), data.get("updated_at")]
    ...
    def _benchmark_table_available(self):
        try:
            return self.db.table_exists("benchmark_result")
        except Exception:
            return False

    def refresh_all(self):
        self.load_models(); self.load_instances()
    ...
    def supported_instance_data(self, data):
        columns = {row[1] for row in self.db.execute("PRAGMA table_info(motor_instance)").fetchall()}
    ...
    def save_instance(self):
        try:
            ...
            if self.current_instance_id is None:
                instance_id = self.instance_repo.create(data); message = f"Motor Instance created: {instance_id}"
            else:
                instance_id = self.current_instance_id; self.instance_repo.update_instance(instance_id, data); message = f"Motor Instance updated: {instance_id}"
            self.current_instance_id = instance_id; self.load_instances(); self.show_instance_detail(instance_id); QMessageBox.information(self, "Complete", message)
    ...
    def open_selected_instance(self, row, _column):
        ...
        self.load_instance_into_form(instance_id); self.show_instance_detail(instance_id); self.tabs.setCurrentWidget(self.detail_tab)
    ...
    def show_instance_detail(self, instance_id):
        ...
        if not data: return
        self.current_instance_id = instance_id
        self.detail_title.setText(f"Instance {instance_id} — {data.get('nickname') or data.get('serial_number') or ''}")
        latest_benchmark = self.instance_repo.get_latest_benchmark(instance_id) if self._benchmark_table_available() else None
        fields = [("Instance ID", data.get("instance_id")), ("Motor Model ID", data.get("motor_model_id")), ("Serial Number", data.get("serial_number")), ("Nickname", data.get("nickname")), ("Status", data.get("status")), ("Health", data.get("health_status")), ("Purchase Date", data.get("purchase_date")), ("Opened Date", data.get("opened_date")), ("Latest Session", data.get("latest_session_id")), ("Latest Benchmark RPM", latest_benchmark.get("benchmark_rpm") if latest_benchmark else None), ("Anomaly Count", data.get("anomaly_count")), ("Consecutive Anomaly", data.get("consecutive_anomaly_count")), ("Created", data.get("created_at")), ("Updated", data.get("updated_at"))]
        ...
        for r, session in enumerate(sessions):
            summary = self.instance_repo.get_breakin_summary(session.get("session_id")) or {}; latest = self.instance_repo.get_latest_log(session.get("session_id")) or {}
            benchmark = self.benchmark_repo.get_by_session(session.get("session_id")) if self._benchmark_table_available() else None
            values = [session.get("session_id"), session.get("device_type"), session.get("device_model"), session.get("start_datetime"), session.get("end_datetime"), session.get("result"), summary.get("log_count", ""), latest.get("measured_rpm", ""), benchmark.get("benchmark_rpm") if benchmark else None, latest.get("current_ma", "")]
    ...
    def compare_selected(self):
        ...
        for row in rows:
            ...
            sessions = self.instance_repo.get_session_history(instance_id); session = sessions[0] if sessions else {}; log = self.instance_repo.get_latest_log(session.get("session_id")) if session.get("session_id") else {}
            benchmark = self.instance_repo.get_latest_benchmark(instance_id) if self._benchmark_table_available() else None
            instances.append((data, session, log or {}, benchmark or {}))
        headers = ["Metric"] + [str(x[0].get("instance_id")) for x in instances]
        metrics = [("Model", lambda d,s,l,b:d.get("motor_model_id")), ("Nickname", lambda d,s,l,b:d.get("nickname")), ("Status", lambda d,s,l,b:d.get("status")), ("Health", lambda d,s,l,b:d.get("health_status")), ("Latest Session", lambda d,s,l,b:s.get("session_id")), ("Latest Result", lambda d,s,l,b:s.get("result")), ("Measured RPM", lambda d,s,l,b:l.get("measured_rpm")), ("Benchmark RPM", lambda d,s,l,b:b.get("benchmark_rpm")), ("Current mA", lambda d,s,l,b:l.get("current_ma")), ("Voltage V", lambda d,s,l,b:l.get("voltage_v")), ("Temperature C", lambda d,s,l,b:l.get("temperature_c")), ("PWM", lambda d,s,l,b:l.get("pwm")), ("Anomaly Count", lambda d,s,l,b:d.get("anomaly_count"))]
        ...
        for r, (name, getter) in enumerate(metrics):
            ...
            for c, instance in enumerate(instances, start=1):
                value = getter(*instance); self.compare_table.setItem(r, c, QTableWidgetItem("" if value is None else str(value)))
...
if __name__ == "__main__":
    ...
    window = MotorManagerUI(); window.show()
    sys.exit(app.exec_())
