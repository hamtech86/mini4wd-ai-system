...
def test_register_raw_log_normalizes_integer_instance_and_session_ids(tmp_path):
    serial = FakeSerial()
    library = FakeLibrary(tmp_path)
    controller = LocalRawLogBreakinController(
        serial_controller=serial,
    ...
    controller.session = FakeSession(session_id=12345)
    controller.active_instance_id = 67890
    controller.active_recipe_name = "STANDARD_3V30S"
    controller._measurement_raw_log_parts = ["RAW\n"]

    record = controller.finalize_benchmark_raw_log()

    assert record.device_instance_id == "67890"
    assert record.motor_id == "67890"
    assert record.measurement_session_id == "12345"
    assert library.read_raw(record.log_id) == "RAW\n"
