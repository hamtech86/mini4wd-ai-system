...
def test_5a_benchmark_extracts_features_without_scoring():
    ...
    assert result["avg_voltage"] == 1.39
    assert result["avg_current"] == 5.0
    assert result["max_current"] == 5.0
    assert result["discharge_time_s"] == 2.0
    assert result["voltage_drop"] == 0.02
    assert result["capacity_mah"] == 2.0 / 3600.0 * 1000.0
...
def test_two_channels_are_averaged_per_measurement():
    ...
    result = analyze_5a_measurements(rows)
    assert result["avg_voltage"] == 1.36
    assert result["avg_current"] == 5.0
    assert result["discharge_time_s"] == 1.0
