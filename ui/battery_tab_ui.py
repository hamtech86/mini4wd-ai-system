...
class BatteryTab(QWidget):
    def __init__(self,db_path,transport=None,parent=None):
        super().__init__(parent); self.db_path=str(db_path); self.transport=transport or BatterySerial(); self.samples={1:[],2:[]}; self.latest_status={1:"IDLE",2:"IDLE"}; self.session_started={1:None,2:None}; self.completed={1:False,2:False}; self._build(); self._load_models_and_instances(); self.timer=QTimer(self); self.timer.setInterval(100); self.timer.timeout.connect(self.poll_serial); self._set_controls_enabled(False)
    ...
    def _build(self):
        ...
        self.register_result_button=QPushButton("REGISTER RESULT TO DATABASE"); self.register_result_button.setEnabled(False); self.register_result_button.clicked.connect(self.register_results); rr.addWidget(self.register_result_button,2,0,1,2); op.addWidget(result); op.addWidget(QLabel("正常終了(COMPLETE)した測定だけを、確認後に手動登録します。STOP/ERROR/CANCELはDB登録しません。")); tabs.addTab(operation,"5A DISCHARGE")
    ...
    def _status(self,ch,value):
        value=str(value).strip().upper(); self.latest_status[ch]=value; self.status_labels[f"CH{ch}"].setText(f"CH{ch}: {value}"); self.status_labels["ALL"].setText(f"ALL: {self.latest_status[1]} / {self.latest_status[2]}")
        if value=="COMPLETE": self.completed[ch]=True; self.result_labels[f"CH{ch}"].setText(f"CH{ch} Result: COMPLETE / {len(self.samples[ch])} samples")
        elif value in ("STOPPED","STOP","CANCEL","CANCELLED","ERROR"): self.completed[ch]=False; self.result_labels[f"CH{ch}"].setText(f"CH{ch} Result: {value} / DB登録不可")
        self._update_register_button()
    def _update_register_button(self): self.register_result_button.setEnabled(any(self.completed[ch] and self.samples[ch] for ch in (1,2)))
    def _same_instance_conflict(self,channel):
        ...
        other=2 if channel==1 else 1; return self.latest_status[other] not in ("IDLE","STOPPED","DISCONNECTED","COMPLETE","ERROR","CANCEL","CANCELLED")
    def start_channel(self,channel):
        if not self.transport.connected: return
        a=self.ch1_instance.currentData(); b=self.ch2_instance.currentData()
        if a is None or b is None: QMessageBox.warning(self,"Battery Instance","CH1 / CH2のBattery Instanceを指定してください。"); return
        if self._same_instance_conflict(channel): QMessageBox.warning(self,"Battery Instance Conflict",f"CH1: {a}\nCH2: {b}\n\n同じBattery Instanceを同時に使用することはできません。\nInstanceを変更してください。"); return
        targets=(1,2) if channel is None else (channel,); now=datetime.now().isoformat(timespec="seconds")
        for ch in targets: self.session_started[ch]=now; self.samples[ch]=[]; self.completed[ch]=False
        if not self.transport.start(channel): QMessageBox.warning(self,"Battery","STARTコマンドを送信できませんでした。"); return
        for ch in targets: self._status(ch,"STARTING")
    def stop_channel(self,channel):
        if not self.transport.connected: return
        if not self.transport.stop(channel): QMessageBox.warning(self,"Battery","STOPコマンドを送信できませんでした。"); return
        for ch in ((1,2) if channel is None else (channel,)): self._status(ch,"STOPPING")
    def poll_serial(self):
        for line in self.transport.read_lines():
            text=line.strip()
            if text.startswith("STATUS"):
                parts=text.split(',')
                if len(parts)>=3: self._status(1,parts[1]); self._status(2,parts[2])
                continue
            sample=self.transport.parse_data(text)
            if sample is None: continue
            ch=sample.channel; prefix=f"CH{ch}"; self.samples[ch].append(sample)
            ...
            state=str(sample.state).strip().upper()
            if state=="COMPLETE": self._status(ch,"COMPLETE")
            elif state in ("ERROR","CANCEL","CANCELLED","STOPPED"): self._status(ch,state)
            else: self.result_labels[prefix].setText(f"{prefix} Result: {len(self.samples[ch])} samples / latest {sample.voltage:.3f} V, {sample.current:.3f} A")
    @staticmethod
    def _stats(samples):
    ...
    @staticmethod
    def _table_columns(db,table): return {row[1] for row in db.execute(f"PRAGMA table_info({table})").fetchall()}
    @staticmethod
    def _insert_available(db,table,values):
        cols=BatteryTab._table_columns(db,table); data={k:v for k,v in values.items() if k in cols}
        if not data: raise sqlite3.OperationalError(f"No compatible columns for {table}")
        names=",".join(data.keys()); marks=",".join("?" for _ in data); db.execute(f"INSERT INTO {table}({names}) VALUES({marks})",tuple(data.values()))
    def register_results(self):
        completed=[]
        for ch in (1,2):
            if not self.completed[ch]: continue
            iid=self.ch1_instance.currentData() if ch==1 else self.ch2_instance.currentData()
            if self.samples[ch] and iid: completed.append((ch,iid,self._stats(self.samples[ch])))
        if not completed: QMessageBox.warning(self,"Battery Result","正常終了(COMPLETE)した測定がありません。"); return
        if QMessageBox.question(self,"Battery Result","COMPLETEした放電結果を確認しました。データベースへ登録しますか?",QMessageBox.Yes|QMessageBox.No,QMessageBox.No)!=QMessageBox.Yes: return
        try:
            with self._db() as db:
                for ch,iid,stats in completed:
                    n,av,ai,ap,mi,mp,dur,cap,energy=stats; sid=int(uuid.uuid4().int%2147483647); start=self.session_started[ch] or datetime.now().isoformat(timespec="seconds"); end=datetime.now().isoformat(timespec="seconds")
                    self._insert_available(db,"measurement_session",{"session_id":sid,"instance_id":iid,"device_type":"EVALUATION","device_model":"BATTERY_5A","firmware_version":"UNKNOWN","start_datetime":start,"end_datetime":end,"operator":"USER","result":"COMPLETE","notes":f"Integrated Battery UI CH{ch}; manual result registration"})
                    for s in self.samples[ch]: self._insert_available(db,"measurement",{"session_id":sid,"record_type":"BATTERY_5A","device_model":"BATTERY_5A","instance_id":iid,"elapsed_time":s.elapsed_sec,"voltage1":s.voltage,"current1":s.current,"pwm":s.pwm,"state":"COMPLETE","power":s.voltage*s.current})
                    self._insert_available(db,"battery_benchmark_result",{"session_id":sid,"instance_id":iid,"analysis_version":"battery-benchmark-v1","measurement_count":n,"avg_voltage":av,"avg_current":ai,"avg_power":ap,"max_current":mi,"max_power":mp,"discharge_time_s":dur,"capacity_mah":cap,"energy_wh":energy})
                db.commit()
            QMessageBox.information(self,"Battery Result","COMPLETEしたMeasurementとBenchmark Resultを登録しました。");
            for ch,_,_ in completed: self.completed[ch]=False; self.result_labels[f"CH{ch}"].setText(f"CH{ch} Result: REGISTERED")
            self._update_register_button()
        except sqlite3.Error as exc: QMessageBox.critical(self,"Battery Result",f"登録できません。\n{exc}")
    def open_database(self):
        BatteryDatabaseDialog(self.db_path,self).exec_(); self._load_models_and_instances()
