...
class BatteryDatabaseDialog(QDialog):
    def __init__(self, db_path: str | Path, parent=None):
        super().__init__(parent); self.db_path=str(db_path); self.setWindowTitle("BATTERY DATABASE / MANUAL REGISTRATION"); self.resize(620,560); self._build(); self._load_models(); self._load_instances(); self._load_sessions()
    def _db(self): return sqlite3.connect(self.db_path)
    def _build(self):
        root=QVBoxLayout(self); tabs=QTabWidget(); root.addWidget(tabs)
        w=QWidget(); f=QFormLayout(w); self.model=QComboBox(); self.iid=QLineEdit(); self.iid.setPlaceholderText("例: BAT0001"); self.serial=QLineEdit(); self.nickname=QLineEdit(); self.notes=QLineEdit()
        ...
        w=QWidget(); root2=QVBoxLayout(w); f=QFormLayout(); self.session=QComboBox(); self.session.setEditable(True); self.instance=QComboBox(); self.version=QLineEdit("battery-benchmark-v1"); self.count=QSpinBox(); self.count.setRange(0,100000000)
        ...
        root2.addLayout(f); q=QGroupBox("CONFIRMATION"); ql=QVBoxLayout(q); self.quality=QPushButton("Measurement quality: NOT CONFIRMED"); self.quality.setCheckable(True); self.quality.clicked.connect(lambda c:self.quality.setText("Measurement quality: OK" if c else "Measurement quality: NOT CONFIRMED")); self.operator=QPushButton("Operator confirmation: NOT CONFIRMED"); self.operator.setCheckable(True); self.operator.clicked.connect(lambda c:self.operator.setText("Operator confirmation: CONFIRMED" if c else "Operator confirmation: NOT CONFIRMED")); ql.addWidget(self.quality); ql.addWidget(self.operator); root2.addWidget(q); b=QPushButton("REGISTER CONFIRMED BENCHMARK RESULT"); b.clicked.connect(self._register_result); root2.addWidget(b); self.status=QLabel("Not registered"); root2.addWidget(self.status); tabs.addTab(w,"2. BENCHMARK RESULT")
    @staticmethod
    def _num(a,b,d): x=QDoubleSpinBox(); x.setRange(a,b); x.setDecimals(d); x.setSingleStep(.1); return x
