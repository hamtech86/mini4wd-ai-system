...
class BatteryDatabaseDialog(QDialog):
    ...
    def _build(self):
        ...
        page=QWidget(); layout=QVBoxLayout(page); form=QFormLayout(); self.model=QComboBox(); self.iid=QLineEdit(); self.serial=QLineEdit(); self.nickname=QLineEdit(); self.notes=QLineEdit(); self.status_combo=QComboBox(); self.status_combo.addItems(["NEW","ACTIVE","RETIRED","ARCHIVED"])
        ...
        self.instance_list=QListWidget(); self.instance_list.itemDoubleClicked.connect(lambda item:self._load_instance_into_form(item.data(32))); layout.addWidget(QLabel("Registered Battery Instances")); layout.addWidget(self.instance_list); tabs.addTab(page,"2. BATTERY INSTANCE")
    ...
    def _load_sessions(self):
        ...
        with self._db() as db:
            ...
            if 'result' in cols and 'end_datetime' in cols: rows=db.execute("SELECT session_id FROM measurement_session WHERE result='COMPLETE' ORDER BY COALESCE(end_datetime,start_datetime,created_at) DESC").fetchall()
            elif 'end_datetime' in cols: rows=db.execute("SELECT session_id FROM measurement_session WHERE end_datetime IS NOT NULL ORDER BY COALESCE(end_datetime,start_datetime,created_at) DESC").fetchall()
            else: rows=db.execute("SELECT session_id FROM measurement_session ORDER BY session_id DESC").fetchall()
        ...
        try:
            with self._db() as db:
                ...
                if 'result' in cols:
                    ...
                    row=db.execute("SELECT end_datetime FROM measurement_session WHERE session_id=?",(sid,)).fetchone(); session_result='COMPLETE' if row and row[0] else 'RUNNING'
                else:
                    row=db.execute("SELECT session_id FROM measurement_session WHERE session_id=?",(sid,)).fetchone(); session_result='COMPLETE' if row else None
