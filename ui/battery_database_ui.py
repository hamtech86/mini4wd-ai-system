"""Operator UI for Battery Instance and confirmed Benchmark registration."""
...
class BatteryDatabaseDialog(QDialog):
    def __init__(self, db_path: str | Path, parent=None):
        ...
        self.setWindowTitle("BATTERY DATABASE / MANUAL REGISTRATION")
        self.resize(620, 560)
        self._ensure_schema()
        self._build()
        self._load_models()
    ...
    def _build(self):
        ...
        for label, widget in (("Battery Model", self.model), ("Instance ID", self.iid), ("Serial Number", self.serial), ("Nickname", self.nickname), ("Notes", self.notes)):
            form.addRow(label, widget)
        register = QPushButton("REGISTER BATTERY INSTANCE")
        register.clicked.connect(self._register_instance)
        form.addRow(register)
        tabs.addTab(page, "1. INSTANCE")

        page = QWidget(); layout = QVBoxLayout(page); form = QFormLayout()
        self.session = QComboBox(); self.session.setEditable(True); self.instance = QComboBox(); self.version = QLineEdit("battery-benchmark-v1")
        ...
        fields = (("Session", self.session), ("Battery Instance", self.instance), ("Analysis Version", self.version),
        ...
        confirm = QGroupBox("CONFIRMATION"); confirm_layout = QVBoxLayout(confirm)
        ...
        confirm_layout.addWidget(self.quality); confirm_layout.addWidget(self.operator); layout.addWidget(confirm)
        register = QPushButton("REGISTER CONFIRMED BENCHMARK RESULT"); register.clicked.connect(self._register_result); layout.addWidget(register)
        self.status = QLabel("Not registered"); layout.addWidget(self.status)
        tabs.addTab(page, "2. BENCHMARK RESULT")

    @staticmethod
    def _num(a, b, decimals):
        widget = QDoubleSpinBox(); widget.setRange(a, b); widget.setDecimals(decimals); return widget
    ...
    def _load_models(self):
        ...
        try:
            ...
            for mid, code, name in rows: self.model.addItem(f"{code} / {name}", mid)
        except sqlite3.Error as exc:
            QMessageBox.warning(self, "Battery Database", f"Battery Modelを読み込めません。\n{exc}")

    def _load_instances(self):
        self.instance.clear()
        try:
            with self._db() as db:
                rows = db.execute("SELECT instance_id,nickname FROM battery_instance WHERE COALESCE(is_deleted,0)=0 ORDER BY instance_id").fetchall()
            for iid, nickname in rows: self.instance.addItem(f"{iid} / {nickname or ''}".rstrip(" /"), iid)
        except sqlite3.Error as exc:
            QMessageBox.warning(self, "Battery Database", f"Battery Instanceを読み込めません。\n{exc}")
    ...
    def _register_instance(self):
        ...
        try:
            with self._db() as db:
                db.execute("INSERT INTO battery_instance(instance_id,battery_model_id,serial_number,nickname,notes) VALUES(?,?,?,?,?)",
                           (iid, self.model.currentData(), self.serial.text().strip() or None, self.nickname.text().strip() or None, self.notes.text().strip() or None))
                db.commit()
            self._load_instances(); QMessageBox.information(self, "Battery Instance", f"{iid} を登録しました。")
        except sqlite3.Error as exc:
            QMessageBox.critical(self, "Battery Instance", f"登録できません。\n{exc}")
    ...
    def _register_result(self):
        ...
        try:
            ...
            with self._db() as db:
                db.execute("INSERT OR REPLACE INTO battery_benchmark_result (session_id,instance_id,analysis_version,measurement_count,avg_voltage,avg_current,avg_power,max_current,max_power,discharge_time_s,voltage_drop,capacity_mah,energy_wh) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)", values)
                db.commit()
