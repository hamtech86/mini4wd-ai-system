...
class BatteryDatabaseDialog(QDialog):
    def __init__(self, db_path: str | Path, parent=None):
        ...
        self._ensure_schema(); self._build(); self._load_models(); self._load_instances(); self._load_sessions(); self.raise_(); self.activateWindow()
    ...
    def _build(self):
        ...
        page=QWidget(); form=QFormLayout(page); self.model=QComboBox(); self.iid=QLineEdit(); self.serial=QLineEdit(); self.nickname=QLineEdit(); self.notes=QLineEdit()
        ...
        b=QPushButton("REGISTER BATTERY INSTANCE"); b.clicked.connect(self._register_instance); form.addRow(b); self.instance_list=QListWidget(); form.addRow("Registered Battery Instances",self.instance_list); tabs.addTab(page,"2. BATTERY INSTANCE")
        page=QWidget(); layout=QVBoxLayout(page); form=QFormLayout(); self.session=QComboBox(); self.session.setEditable(True); self.instance=QComboBox(); self.version=QLineEdit("battery-benchmark-v1"); self.count=QSpinBox(); self.count.setRange(0,100000000); self.avg_v=self._num(0,10,4); self.avg_i=self._num(0,100,4); self.avg_p=self._num(0,1000,4); self.max_i=self._num(0,100,4); self.max_p=self._num(0,1000,4); self.duration=self._num(0,1000000,2); self.drop=self._num(-10,10,4); self.cap=self._num(0,100000,3); self.energy=self._num(0,100000,3)
    ...
    def _load_instances(self):
        self.instance.clear(); self.instance_list.clear(); self.model.clear()
        try:
            with self._db() as db:
            ...
            for iid,serial,nickname,code in rows:
                text=f"{iid} | {code or '-'} | {nickname or '-'} | SN:{serial or '-'}"; self.instance.addItem(text,iid); self.instance_list.addItem(text)
        except sqlite3.Error as exc: QMessageBox.warning(self,"Battery Database",f"Battery Instanceを読み込めません。\n{exc}")
    def _load_sessions(self):
        self.session.clear()
        try:
            with self._db() as db: rows=db.execute("SELECT session_id FROM measurement_session WHERE status='COMPLETE' ORDER BY start_time DESC").fetchall()
