...
class MainWindow(QMainWindow):
    ...
    def __init__(self,context=None):
        ...
        self.setWindowTitle("MINI4WD AI SYSTEM - MOTOR BREAKIN V3"); self.resize(820,900); self.setMinimumSize(720,560); self.build_ui(); self.load_recipes(); self.load_instances(); self.set_ready()
    def build_ui(self):
        ...
        content=QWidget(); content.setMinimumWidth(0); main=QVBoxLayout(content); main.setContentsMargins(6,5,6,8); main.setSpacing(7)
        head=QHBoxLayout(); h=QLabel("MOTOR BREAK-IN SYSTEM"); h.setStyleSheet("font-size:25px;font-weight:bold;"); head.addWidget(h); head.addStretch(); self.stop=QPushButton("EMERGENCY STOP"); self.stop.setEnabled(False); self.stop.setFixedHeight(28); self.stop.setFixedWidth(125); self.stop.clicked.connect(self.stop_run); head.addWidget(self.stop); main.addLayout(head)
        ...
        for k,w in self.progress.items(): box=QGroupBox(k); q=QVBoxLayout(box); q.setContentsMargins(5,4,5,4); w.setStyleSheet("font-size:14px;font-weight:bold;"); w.setAlignment(Qt.AlignCenter); w.setMinimumHeight(30); q.addWidget(w); r.addWidget(box,1)
        rv.addLayout(r)
        ...
        for k,w in self.result.items(): box=QGroupBox(k); q=QVBoxLayout(box); q.setContentsMargins(5,4,5,4); w.setStyleSheet("font-size:14px;font-weight:bold;"); w.setWordWrap(True); w.setSizePolicy(QSizePolicy.Expanding,QSizePolicy.Preferred); q.addWidget(w); r.addWidget(box,1)
        rv.addLayout(r); self.copy=QPushButton("COPY RESULT"); self.copy.setEnabled(False); self.copy.setMinimumHeight(34); self.copy.clicked.connect(self.copy_result); rv.addWidget(self.copy); main.addWidget(result)
