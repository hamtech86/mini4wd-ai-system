...
class BreakinWorker(QThread):
    ...
    def run(self):
        try:
            self.completed.emit(self.controller.benchmark_3v(duration_sec=30) if self.benchmark else self.controller.start(self.recipe))
        except Exception as exc: self.failed.emit(f"{type(exc).__name__}: {exc}")

class MainWindow(QMainWindow):
    BENCHMARK_KEY = "__MOTOR_BENCHMARK_TEST__"
    def __init__(self, context=None):
        super().__init__(); self.context=context; self.breakin_worker=None; self.last_result_data=None; self.last_result_benchmark=False; self.database_updated=False
        root=Path(__file__).resolve().parent.parent; self.db_path=root/"database"/"mini4wd.db"; self.recipe_engine=RecipeEngine(str(root/"config"/"breakin_recipes.yaml"))
    ...
    def build_ui(self):
        outer=QWidget(); outer_layout=QVBoxLayout(outer); outer_layout.setContentsMargins(3,3,3,3); scroll=QScrollArea(); scroll.setWidgetResizable(True); scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff); scroll.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOn); scroll.setFrameShape(QScrollArea.NoFrame)
        ...
        head=QHBoxLayout(); title=QLabel("MOTOR BREAK-IN SYSTEM"); title.setStyleSheet("font-size:25px;font-weight:bold;"); head.addWidget(title); head.addStretch(); self.stop=QPushButton("EMERGENCY STOP"); self.stop.setEnabled(False); self.stop.setFixedSize(120,28); self.stop.clicked.connect(self.stop_run); head.addWidget(self.stop); main.addLayout(head)
        pre=QGroupBox("① 駆動前：操作"); pv=QVBoxLayout(pre); r=QHBoxLayout(); r.addWidget(QLabel("MOTOR INSTANCE")); self.instance=QComboBox(); self.instance.setSizePolicy(QSizePolicy.Expanding,QSizePolicy.Fixed); r.addWidget(self.instance,1); self.instance_id=QLabel("ID: --"); self.instance_id.setFixedWidth(55); r.addWidget(self.instance_id); self.manager=QPushButton("INSTANCE MANAGER"); self.manager.setFixedWidth(145); self.manager.clicked.connect(self.open_manager); r.addWidget(self.manager); pv.addLayout(r)
        r=QHBoxLayout(); r.addWidget(QLabel("RECIPE")); self.recipe=QComboBox(); self.recipe.setSizePolicy(QSizePolicy.Expanding,QSizePolicy.Fixed); self.recipe.currentIndexChanged.connect(self.recipe_changed); r.addWidget(self.recipe,1); self.start=QPushButton("START BREAK-IN"); self.start.setFixedWidth(145); self.start.clicked.connect(self.start_run); r.addWidget(self.start); pv.addLayout(r); self.description=QLabel("-"); self.description.setWordWrap(True); pv.addWidget(self.description); main.addWidget(pre)
        run=QGroupBox("② 駆動中：プログレス / LIVE DATA"); rv=QVBoxLayout(run); self.run_state=QLabel("READY"); self.run_state.setStyleSheet("font-size:22px;font-weight:bold;"); rv.addWidget(self.run_state); self.progress={k:QLabel("--") for k in ("STEP","PHASE","DIR","PWM","VOLT","CURRENT","ELAPSED","REMAIN")}; grid=QGridLayout()
        for i,key in enumerate(self.progress): box=QGroupBox(key); q=QVBoxLayout(box); self.progress[key].setStyleSheet("font-size:14px;font-weight:bold;"); self.progress[key].setAlignment(Qt.AlignCenter); self.progress[key].setMinimumHeight(34); q.addWidget(self.progress[key]); grid.addWidget(box,i//4,i%4)
        ...
        detail=QGroupBox("SELECTED RECIPE"); dv=QVBoxLayout(detail); line=QHBoxLayout(); self.recipe_detail=QLabel("-"); self.benchmark_detail=QLabel("-"); line.addWidget(QLabel("Description:")); line.addWidget(self.recipe_detail,1); line.addWidget(QLabel("Benchmark:")); line.addWidget(self.benchmark_detail,1); dv.addLayout(line); self.phase_detail=QLabel("-"); dv.addWidget(self.phase_detail); main.addWidget(detail)
    ...
    def load_instances(self):
        ...
        try:
            ...
            rows=conn.execute("SELECT mi.instance_id, mi.motor_model_id, mi.serial_number, mi.nickname, mm.name, mm.series FROM motor_instance mi LEFT JOIN motor_model mm ON mm.motor_model_id=mi.motor_model_id WHERE COALESCE(mi.is_deleted,0)=0 ORDER BY mi.instance_id DESC").fetchall(); conn.close()
    ...
    def recipe_changed(self,_):
        ...
        if name==self.BENCHMARK_KEY: self.recipe_detail.setText("3.00 V closed-loop benchmark / 30 s / brush peak measurement"); self.phase_detail.setText("BENCHMARK_3V_TEST → BRUSH PEAK"); self.benchmark_detail.setText("3.00 V / 30 s"); return
    ...
    def refresh_runtime(self):
        ...
        elapsed=float(c.phase_elapsed_sec()) if hasattr(c,"phase_elapsed_sec") else 0.0; duration=float(getattr(phase,"duration_sec",0)); index=int(getattr(c,"current_phase_index",0)); total=int(getattr(c,"total_phases",0)); values={"STEP":f"{index+1} / {total}","PHASE":getattr(phase,"name","--"),"DIR":getattr(phase,"direction","FWD"),"PWM":getattr(c,"current_pwm",0),"VOLT":self.live["V"].text(),"CURRENT":self.live["A"].text(),"ELAPSED":f"{elapsed:.1f} s","REMAIN":f"{max(0,duration-elapsed):.1f} s"}
    ...
    def complete(self,data,benchmark):
        self.timer.stop(); self.refresh_runtime(); self.run_state.setText("COMPLETE"); self.last_result_data=data; self.last_result_benchmark=benchmark; self.database_updated=False; self.result["STATUS"].setText("BENCHMARK COMPLETE" if benchmark else "BREAK-IN COMPLETE"); rpm=self._latest_rpm(); self.result["RPM"].setText(f"{rpm:.0f} rpm" if rpm is not None else "NOT AVAILABLE"); self.result["BENCHMARK RPM"].setText(self.benchmark_rpm_input.text().strip()+" rpm" if self.benchmark_rpm_input.text().strip() else "NOT ENTERED"); controller_peak=getattr(self.breakin_controller,"last_brush_peak_current",None); peak=self._number(data,"brush_peak_current","peak_current","peak_current_a","max_current","current_peak"); peak=controller_peak if peak is None else peak; self.result["BRUSH PEAK"].setText(f"{peak:.3f} A" if peak is not None else "NOT RECORDED"); measurement=getattr(getattr(self.breakin_controller,"measurement_manager",None),"last_measurement",None); self.result["VOLTAGE"].setText(f"{float(getattr(measurement,'motor_voltage',0)):.2f} V" if measurement else "--"); self.result["CURRENT"].setText(f"{float(getattr(measurement,'current_avg',0)):.3f} A" if measurement else "--"); self.result["PWM"].setText(str(getattr(measurement,"pwm",getattr(self.breakin_controller,"current_pwm","--"))) if measurement else "--"); self.result["DIRECTION"].setText(str(getattr(measurement,"direction",getattr(self.breakin_controller,"serial",None).direction if getattr(self.breakin_controller,"serial",None) else "--"))); self.result["BENCHMARK"].setText("3.00 V / 30 s" if benchmark else "--"); self.result["SUMMARY"].setText("ベンチマーク測定完了" if benchmark else "ブレイクイン完了"); self.copy.setEnabled(True); self.update_db.setEnabled(self.instance.currentData() is not None); self.database_status.setText("DATABASE: NOT UPDATED / 結果確認後に更新")
    def failed(self,message):
        self.timer.stop(); self.run_state.setText("ERROR"); self.result["STATUS"].setText("ERROR"); self.result["SUMMARY"].setText(message); self.copy.setEnabled(True); self.update_db.setEnabled(False); self.database_status.setText("DATABASE: NOT UPDATED / ERROR")
    def finished(self):
    ...
    def copy_result(self):
        text="\n".join(f"{key}: {widget.text()}" for key,widget in self.result.items()); QApplication.clipboard().setText(text); self.copy.setText("COPIED")
