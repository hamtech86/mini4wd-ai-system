...
def main():
    ...
    try:
        ...
        required = {"session_id", "elapsed_time", "voltage1"}
        missing = required - measurement_cols
        if missing:
            raise RuntimeError(
                "measurement schema is missing required columns: " + ", ".join(sorted(missing))
            )

        db.executescript(TRIGGER_SQL)

        # Backfill existing Benchmark Result rows from the raw Measurement log.
        rows = db.execute(
            "SELECT result_id, session_id FROM battery_benchmark_result"
        ).fetchall()
        updated = 0
        for result_id, session_id in rows:
            ...
            if first is None or last is None:
                continue
            start_voltage = float(first[0])
            end_voltage = float(last[0])
            db.execute(
                """UPDATE battery_benchmark_result
                   SET start_voltage=?, end_voltage=?, voltage_drop=?
                   WHERE result_id=?""",
        ...
        db.commit()
        print(
            f"Battery Benchmark v2 ready. Existing results backfilled: {updated}/{len(rows)}"
        )
    finally:
        db.close()
