...
class BatterySerial:
    ...
    def read_lines(self):
        if not self.connected or not self.serial: return []
        lines=[]
        while self.serial.in_waiting:
            text=self.serial.readline().decode("utf-8",errors="replace").strip()
    ...
    def parse_data(line):
        ...
        try:
            ...
            elapsed_ms=float(fields[3]); current=float(fields[4]); voltage=float(fields[5])
            pwm=int(float(fields[7])); state=fields[9].strip() or "--"
        except (ValueError,IndexError): return None
        return BatterySample(channel=channel,voltage=voltage,current=current,pwm=pwm,elapsed_sec=elapsed_ms/1000.0,state=state,raw=line)
