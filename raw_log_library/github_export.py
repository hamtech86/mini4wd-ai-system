...
This module is deliberately an outbound adapter. It reads an already persisted
Raw Log and its management metadata, then publishes an immutable copy to the
configured GitHub repository only when explicitly requested by the UI.

Authentication is supplied by the runtime environment; no token is stored in
the repository or in Raw Log data.
"""
...
class GitHubRegistrationError(RuntimeError):
    """Raised when a manual Raw Log registration cannot be completed."""


class GitHubRawLogExporter:
    """Publish one immutable Local Raw Log to GitHub."""
    ...
    def register(self, log_id: str) -> Dict[str, Any]:
        if not self.token:
            raise GitHubRegistrationError(
                f"GitHub token is not configured. Set {TOKEN_ENV} in the application environment."
            )
        ...
        raw_rel = self._raw_path(record)
        metadata_rel = self._metadata_path(record)

        metadata = asdict(record)
        metadata["raw_source"] = str(raw_path)
        metadata["registered_at"] = datetime.now(timezone.utc).isoformat()
        metadata["repository"] = self.repository
        metadata["raw_path"] = raw_rel
        metadata["metadata_path"] = metadata_rel

        try:
            raw_result = self._put_file(
                raw_rel, raw_body, f"Register Raw Log {log_id}", allow_update=False
            )
            metadata_result = self._put_file(
                metadata_rel,
                json.dumps(metadata, ensure_ascii=False, indent=2) + "\n",
                f"Register Metadata {log_id}",
                allow_update=False,
            )
    ...
    def _metadata_path(self, record) -> str:
        kind = record.device_type.lower()
        individual_id = record.motor_id if kind == "motor" else record.battery_id
        if not individual_id:
            raise GitHubRegistrationError(f"{record.log_id}: individual ID is missing.")
    ...
    def _put_file(self, path: str, content: str, message: str, *, allow_update: bool) -> Dict[str, Any]:
        ...
        if existing_sha:
            if not allow_update:
                raise GitHubRegistrationError(
                    f"GitHub target already exists: {path}. Duplicate registration was blocked."
                )
        ...
        try:
            ...
            raise GitHubRegistrationError(
                f"GitHub API error {exc.code} for {path}: {detail}"
            ) from exc
        except urllib.error.URLError as exc:
            raise GitHubRegistrationError(
                f"GitHub network error for {path}: {exc.reason}"
            ) from exc

    def _existing_sha(self, url: str) -> str | None:
        request = urllib.request.Request(
            url,
            method="GET",
            headers={
        ...
        try:
            ...
            detail = exc.read().decode("utf-8", errors="replace")
            raise GitHubRegistrationError(
                f"GitHub lookup error {exc.code}: {detail}"
            ) from exc
        except urllib.error.URLError as exc:
            raise GitHubRegistrationError(
                f"GitHub network error during lookup: {exc.reason}"
            ) from exc
