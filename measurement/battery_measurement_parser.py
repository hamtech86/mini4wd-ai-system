...
STARTALL starts CH1 and CH2 independently at the same command event. The
firmware then emits separate CH1 and CH2 DATA frames. Therefore ALL is a
control operation, not a Measurement channel.
"""
...
def parse_battery_data_frame(raw: str | bytes) -> Measurement:
    ...
    return Measurement(
        ...
        power=power, current_ripple=0.0, voltage_ripple=0.0,
        peak_power=power, peak_current=current, peak_voltage=voltage,
        peak_pwm=pwm, brush_peak_current=0.0, raw_magnetic=0,
        magnetic_level=0.0, motor_temperature=0.0,
